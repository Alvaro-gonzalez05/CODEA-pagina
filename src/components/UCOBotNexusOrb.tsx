...
const IconWrapper = ({
  ...
  <div
    className={`
            ...
            ${
              isHighlighted
                ? "dark:bg-cyan-700/50 bg-cyan-100/80 border-cyan-400/50 dark:shadow-cyan-500/20 shadow-cyan-400/30 shadow-2xl animate-breathing-glow"
...
const UCOBotIconGrid = () => {
  ...
  const outerIcons = useMemo(
    (): OuterIcon[] => [
      { id: 1, component: <WhatsAppLogo /> },
      { id: 2, component: <ClockLogo /> },
      { id: 3, component: <GlobeLogo /> },
      { id: 4, component: <ZapLogo /> },
      { id: 5, component: <RestaurantLogo /> },
      { id: 6, component: <HotelLogo /> },
      { id: 7, component: <RetailLogo /> },
      { id: 8, component: <ChatLogo /> },
    ],
    [],
  );
    ...
    const render = () => {
      ...
      particlesRef.current.forEach((p, index) => {
        ...
        if (p.life <= 0) {
          ...
          ctx.fillStyle = `rgba(6, 182, 212, ${p.life / 60})`;
          ctx.fill();
        }
      });
            ...
            return outerIcons.map((icon2, j) => {
              ...
              return (
                <line
                  ...
                  strokeWidth="1.5"
                  style={{
                    stroke: isLineActive ? "#06B6D4" : "#6B7280",
                    opacity: isLineActive ? 0.8 : 0.15,
                    filter: isLineActive ? "url(#glow_ucobot)" : "none",
          ...
          return (
            ...
            >
              <div className="-translate-x-1/2 -translate-y-1/2 relative">
                <div
                  className="absolute inset-[-20px] bg-cyan-500/20 dark:bg-cyan-500/30 rounded-full blur-2xl"
                  style={{
...
export default function UCOBotNexusOrb() {
  return (
    <div className="w-full flex items-center justify-center font-sans p-4 sm:p-8 overflow-hidden">
      <style>
        {`
                ...
                @keyframes breathing-glow {
                    0% { box-shadow: 0 0 20px 0px rgba(6, 182, 212, 0.3); filter: drop-shadow(0 0 5px rgba(6, 182, 212, 0.2)); }
                    50% { box-shadow: 0 0 35px 10px rgba(6, 182, 212, 0.1); filter: drop-shadow(0 0 15px rgba(6, 182, 212, 0.1)); }
                    100% { box-shadow: 0 0 20px 0px rgba(6, 182, 212, 0.3); filter: drop-shadow(0 0 5px rgba(6, 182, 212, 0.2)); }
                }
                @keyframes breathing-glow-light {
                    0% { box-shadow: 0 0 20px 0px rgba(6, 182, 212, 0.2); filter: drop-shadow(0 0 5px rgba(6, 182, 212, 0.1)); }
                    50% { box-shadow: 0 0 35px 10px rgba(6, 182, 212, 0.05); filter: drop-shadow(0 0 15px rgba(6, 182, 212, 0.05)); }
                    100% { box-shadow: 0 0 20px 0px rgba(6, 182, 212, 0.2); filter: drop-shadow(0 0 5px rgba(6, 182, 212, 0.1)); }
                }
      ...
      <div className="absolute inset-0 z-0">
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_80%_80%_at_50%_-20%,rgba(6,182,212,0.1),rgba(255,255,255,0))] dark:bg-[radial-gradient(ellipse_80%_80%_at_50%_-20%,rgba(6,182,212,0.3),rgba(255,255,255,0))]"></div>
      </div>
